...
public class GayleShapley {
    static int N;

    public GayleShapley(int n){
        N=n;
    }

    // This function returns true if woman
// 'w' prefers man 'm1' over man 'm'
     public boolean wPrefersM1OverM(int[][] prefer, int w,
                                    int m, int m1)
    {
...
// Print the solution
        OtherAlgo.ret.append("Woman\tMan\n");
        for (int i = 0; i < N; i++)
        {
            OtherAlgo.ret.append(" ").append(i+N).append("\t").append(wPartner[i]).append("\n");
        }
    }
